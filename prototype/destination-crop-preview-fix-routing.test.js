...
// Smoke test: destination-crop-preview must route each crop issue to the screen that owns the fix (#583).
...
//   node prototype/destination-crop-preview-fix-routing.test.js
...
const source = fs.readFileSync(path.join(root, "prototype", "destination-crop-preview.html"), "utf8");
...
for (const { key, file } of routes) {
  assert.ok(source.includes(`${key}: "${file}"`), `destination-crop-preview routes ${key} to ${file}`);
...
console.log("destination-crop-preview: " + routes.length + " flagged conditions route to " + targets.length + " owning fix screens");
